...
init(convert=True, autoreset=True) # Resetar as cores nas outras linhas
...
######################################
''')
tipo_de_conta = input(' Defina a Conta que deseja Operar!\n REAL\n PRACTICE\n :: ').upper()

try:
	try_login = 0
	iq = {}
	status = False
	print('\n\nInfome seu usuario e senha para iniciar o robo\n')
	while status == False and try_login < 3:
		...
		if status == False:
			...
			if "code" in res and res["code"] == "invalid_credentials":
				print("\n\nErro ao conectar: Usuario ou senha incorreta!\n\n")
			else:
			...
			break
	if status == False:
		raise ValueError("Voce excedeu 3 tentativas. Tente Novamente mais tarde!\n\n")
	...
	if entrar:
		...
		if dir:
			...
			for i in range(martingale):
				...
				status,id = iq.buy_digital_spot(par, valor_entrada, dir, 1) if operacao == 1 else iq.buy(valor_entrada, par, dir, 1)
				
				if status:
					while True:
						try:
							status,valor = iq.check_win_digital_v2(id) if operacao == 1 else iq.check_win_v3(id)
						except:
							status = True
							valor = 0
